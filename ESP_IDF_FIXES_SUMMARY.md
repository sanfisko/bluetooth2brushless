...
- **Проблема**: Устаревшие инструкции с `get_idf` алиасом
- **Причина**: Алиас может не работать, создает путаницу
- **Решение**: Полностью переписана документация под ESP-IDF v5.4.1 с прямыми командами

## Результат:
Документация полностью обновлена для ESP-IDF v5.4.1:
- Убраны все упоминания устаревшего `get_idf` алиаса
- Добавлены четкие инструкции по активации окружения
- Исправлены все проблемы компиляции для ESP-IDF v5.4.1
- Добавлено подробное troubleshooting